...
const Settings = () => {
      ...
      const fetchUserData = async () => {
        try {
          ...
          if (dbUser && dbUser.email) {
            ...
            // Fallback to Clerk email if database email is not available
          ...
          if (userSettings && !userSettings.message) {
  ...
  const handleAddCustomCategory = () => {
    if (customCategory.trim() !== "" && !formData.categories.includes(customCategory.trim())) {
      setFormData(prevData => ({
        ...prevData,
        categories: [...prevData.categories.filter(c => c !== ""), customCategory.trim()]
      }));
      setCustomCategory("");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Save settings to backend (API keys will be encrypted)
